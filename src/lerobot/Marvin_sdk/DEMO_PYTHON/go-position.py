...
robot.set_vel_acc(arm='B',velRatio=80, AccRatio=80)
robot.send_cmd()
time.sleep(1)
...
while 1:
  '''点位0'''
  robot.clear_set()
  joint_cmd_1=[-150.,-70.,120.,30.,-120.,30.,-30.]
  robot.set_joint_cmd_pose(arm='B',joints=joint_cmd_1)
  robot.send_cmd()
  time.sleep(1) #预留运动时间
  sub_data = robot.subscribe(dcss)
  print({sub_data['outputs'][1]['low_speed_flag']})
  while sub_data['outputs'][1]['low_speed_flag'] != b'\x01':
        sub_data = robot.subscribe(dcss)
        print({sub_data['outputs'][1]['low_speed_flag']})
        time.sleep(0.1) #预留运动时间

  '''点位1'''
  robot.clear_set()
  joint_cmd_1=[150.,70.,-120.,-90.,120.,-30.,30.]
  robot.set_joint_cmd_pose(arm='B',joints=joint_cmd_1)
  robot.send_cmd()
  time.sleep(1) #预留运动时间
  sub_data = robot.subscribe(dcss)
