...
def main():
    ...
    while not stop_flag:
        ...
        # 查看A臂关节位置
        a_joints = sub_data['outputs'][0]['fb_joint_pos']
        print(f"current joints of arm A : "
              f"{a_joints[0]:.3f} {a_joints[1]:.3f} {a_joints[2]:.3f} "
              f"{a_joints[3]:.3f} {a_joints[4]:.3f} {a_joints[5]:.3f} "
              f"{a_joints[6]:.3f}")

        # 查看B臂关节位置
        b_joints = sub_data['outputs'][1]['fb_joint_pos']
        print(f"current joints of arm B : "
              f"{b_joints[0]:.3f} {b_joints[1]:.3f} {b_joints[2]:.3f} "
              f"{b_joints[3]:.3f} {b_joints[4]:.3f} {b_joints[5]:.3f} "
              f"{b_joints[6]:.3f}")
        ...
        print("===== M1主动拖动 | M2跟随 | 力反馈夹持感 =====")
        print(f"M1 位置: {m1_pos:.3f} | 反馈力矩: {feedback_tau:.3f} | "
              f"状态: {err_to_str(Motor1.getErr())}")
        print(f"M2 位置: {m2_pos:.3f} | 目标: {m2_target:.3f} | "
              f"夹持力: {m2_tau:.3f} | 状态: {err_to_str(Motor2.getErr())}")
        print("================================================")
