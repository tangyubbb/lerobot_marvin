...
class OpenCVCamera(Camera):
    ...
    def _configure_capture_settings(self) -> None:
        ...
        # Set resolution first, then FOURCC, then FPS
        # This order is important for V4L2 drivers where (resolution, FOURCC) determines available FPS
        ...
        # Step 2: Set FOURCC (after resolution, as it depends on resolution)
        if self.config.fourcc is not None:
            self._validate_fourcc()

        # Step 3: Set FPS (after resolution and FOURCC, as available FPS depends on both)
        if self.fps is None:
    ...
    def _validate_width_and_height(self) -> None:
        ...
        # 放宽验证：只要实际分辨率匹配即可，不强制 set() 返回 True
